#/usr/bin/env python
#This code was forked on 4/21/16 and became rfipowspec. Instead
# of dropping frames around a reset, it replaces them with blank
# ones in order to keep the timing consistent.
#updated 11/17/15 8 PM
...
dir = 'images/'
filename = 'saphira_14:46:36.269917196.fits'
print "Reading image..."
    ...
    for i in np.arange(0, np.shape(img)[0]-1): #1000):
        if (i not in resets) & (i+1 not in resets):
            if 0: #simulate noise to see how it propagates
                ...
                done[i] = flux_direction * (img[i, :, :] - img[i+1, :, :])
                if 0:
...
def findrfi(powspec, xaxis, psf):
    ...
    if psf==True:
        ...
        #find indices of local maxima of identifieda
...
def get_resets_2(img):
    ...
    if 1:
        ...
        plt.plot(meds[:100], 'o')
        plt.axhline(y=10.*std)
        plt.xlabel('frame number')
        plt.ylabel('CDS median')
        plt.show()
    ...
    while (i < np.shape(img)[0]) & (cont == True): #kludgy for loop with varying increments
        if i in reset_guesses: #if the reset was detected previously
            ...
            #just wasn't detected previously, but freq hasn't changed
            if i+reset_freq in reset_guesses:
                resets_actual = np.append(resets_actual, i)
                ...
                if i-1 in reset_guesses:
                    ...
                    print "Bunch of frames dropped around "+str(int(i))+". Giving up."
                    cont = False
                    #print "Code shouldn't have reached here."
                    plt.plot(np.arange(i-50, i+50), meds[i-50:i+50], 'o')
                    plt.axhline(y=10.*std)
                    plt.show()
                    pdb.set_trace()
        i += reset_freq
