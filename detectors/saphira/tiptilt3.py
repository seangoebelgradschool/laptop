...
def align(file):
    ...
    n_brightest = 30 #number of pixels to include in centroid calculation
    brightness_threshold = 400 #when do you stop looking for an astrometric speckle?
    #1600 for 12:16:42
...
### ALIGN IMAGES
    ...
    for i in range(0, np.shape(img)[0]):
        ...
        x_arr, y_arr, weight = get_centroids(diff, mask_q1, mask_q2, mask_q3, mask_q4, \
                                             ...
                                             q1_y0, q2_x0, q2_y0, q3_x0, q3_y0, q4_x0, \
                                             q4_y0, i)
...
def get_centroids(diff, mask_q1, mask_q2, mask_q3, mask_q4, brightness_threshold, \
                  n_brightest, q1_x0, q1_y0, q2_x0, q2_y0, q3_x0, q3_y0, q4_x0, q4_y0, i):
    xarr = []
    yarr = []
    wt = []
    ...
    #Q1
    im = diff*mask_q1
    if np.sum(im) < 0: im *= -1 #invert if negative
    #print np.max(im)
