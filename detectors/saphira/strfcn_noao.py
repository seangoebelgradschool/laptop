...
# Forked from strfcn2 on 8/31. No longer uses 29 pixel mask for calculating centroids.
...
dir = 'images/' #../logs/'
filename = '150729_233701_cleaned.fits'#150729_230025_cleaned.fits'

global filename, dir

def both(): #aligns image. Then computes structure function for it.
    align()
    strfcn()
...
def align():
    ...
    im_coadd -= np.median(im_coadd[:,80:]) #subtract off median of empty right half of image
    ...
    #identify hot pixels
    bright = im_coadd > np.sort(im_coadd.flatten())[np.size(im_coadd)*0.95]
    hotpix = find_hot_pixels(im_coadd*bright)
    ...
    for i in range(0, np.shape(img)[0]):
        ...
        im = hot_pixel_clean(hotpix, im) #clean hot pixels
    
        im -= np.median(im[:,80:])

        bright = im > np.sort(im.flatten())[np.size(im)*0.95]

        x1 = np.sum(np.sum(im*bright, 0)*range(np.shape(im)[1]))/np.sum(im*bright)
        y1 = np.sum(np.sum(im*bright, 1)*range(np.shape(im)[0]))/np.sum(im*bright)

        img[i,:,:] = scipy.ndimage.interpolation.shift(im, [y0-y1, x0-x1], mode='wrap')
        #plt.imshow(im*bright, interpolation='none')
        #plt.show()
...
######################
    ...
    img = pyfits.getdata(dir+filename[:filename.find('.fits')] + 'aligned.fits')
    #pdb.set_trace()

    #Calculate approximate center of PSF and find avg intensities
    im_coadd = np.sum(img,0)
    selection = im_coadd > np.sort(im_coadd.flatten())[np.size(im_coadd) * 0.67] #brightest 1/3
    ...
    img_var = np.zeros((20,40))
    img_time = np.zeros((20, 40))
    img_counts = np.zeros((20,40))
